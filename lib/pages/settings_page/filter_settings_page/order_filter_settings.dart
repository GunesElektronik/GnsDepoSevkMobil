...
class OrderFilterSettingsPage extends StatefulWidget {
  const OrderFilterSettingsPage({super.key});

  @override
  State<OrderFilterSettingsPage> createState() =>
      _OrderFilterSettingsPageState();
}

class _OrderFilterSettingsPageState extends State<OrderFilterSettingsPage> {
  ...
  void _getSharedPreferences() async {
    ...
    setState(() {
      isOnlyIsAssingFalse = sharedPreferences
              .getBool(sharedPreferencesKey.isOnlyIsAssingFalseForOrder) ??
          false;
      ordersAssingedToMe =
          sharedPreferences.getBool(sharedPreferencesKey.ordersAssingedToMe) ??
              false;
        ...
        children: [
          ...
          Card(
            ...
            child: ListTile(
              ...
              title: const Text(
                "Sadece Bana Atanmış Siparişleri Göster",
                style: TextStyle(fontWeight: FontWeight.w500),
              ),
              contentPadding:
                  const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
  ...
  void _saveIsOnlyIsAssingFalseData() {
    sharedPreferences.setBool(
        sharedPreferencesKey.isOnlyIsAssingFalseForOrder, isOnlyIsAssingFalse);
    sharedPreferences.setBool(
        sharedPreferencesKey.ordersAssingedToMe, ordersAssingedToMe);
