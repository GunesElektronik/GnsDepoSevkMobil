...
import 'package:gns_warehouse/pages/create_order/create_purchase_order/create_purchase_product_update.dart';
...
class _CreatePurchaseOrderState extends State<CreatePurchaseOrder> {
    ...
    try {
      ...
      if (response != null) {
        if (response.products!.items!.isNotEmpty) {
          var newItem = PurchaseOrderItems(
            orderItemId: "",
    ...
    return PopScope(
      ...
      child: SafeArea(
        child: Scaffold(
          ...
          appBar: GnsAppBar("Satın Alma Siparişi Oluştur", context),
          body: isFetched
              ? Container(
                  child: Padding(
  ...
  Padding _createPurchaseOrderButton() {
    return Padding(
...
// 24 Hour format:
      ...
      builder: (context) => PopScope(
        ...
        child: AlertDialog(
            ...
            content: SingleChildScrollView(
              child: CreatePurchaseOrderProductUpdate(
  ...
  Future<CreatePurchaseOrderRequest> _createPurchaseOrderBody() async {
    return CreatePurchaseOrderRequest(
      ...
      erpCode: "",
      projectId: projectId,
      orderItems: _createPurchaseOrderItemList(),
    );
  }
  ...
  List<PurchaseOrderItems> _createPurchaseOrderItemList() {
    List<PurchaseOrderItems> salesOrderItemsList = [];
    ...
    for (int j = 0; j < salesProductList.length; j++) {
      ...
      salesOrderItemsList.add(
        PurchaseOrderItems(
          ...
          orderitemtypeid: element.bodyItem.orderitemtypeid,
          projectId: projectId,
        ),
      );
    }
...
class ProductDetailAndScannedNumberForPurchaseOrderCreate {
  ProductListReponse response;
  PurchaseOrderItems bodyItem;
  int scannedNumber;
