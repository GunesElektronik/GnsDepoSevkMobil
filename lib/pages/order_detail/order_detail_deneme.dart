...
import 'package:gns_warehouse/pages/order_detail/order_scan_product.dart';
...
class OrderDetailDeneme extends StatefulWidget {
  const OrderDetailDeneme(
      {super.key, required this.orderId, required this.item});
  final String orderId;
  final OrderSummaryItem item;
  @override
  State<OrderDetailDeneme> createState() => _OrderDetailDenemeState();
}

class _OrderDetailDenemeState extends State<OrderDetailDeneme>
  ...
  void _saveOrderSummaryOnLocal() async {
    await _dbHelper.updateOrderHeader(_createOrderSummarLocalModel());
  }
  ...
  Future<void> _setOrderAssingStatus(bool isAssingRemote) async {
    ...
    bool isSuccess = await apiRepository.setOrderAssingStatus(
        isAssingRemote, apiRepository.employeeUid, widget.orderId);
    ...
    if (isSuccess) {
      ...
      if (isAssingRemote) {
        response.data!.orderItems!.forEach((element) async {
          await _dbHelper.addOrderDetailItem(OrderDetailItemDB(
            orderId: widget.orderId,
            orderItemId: element.id,
            productId: element.product!.id!,
            //warehouseId: element.warehouse!.id, olması gereken bu
            warehouseId: response.data!.warehouse!.id,
            ficheNo: response.data!.ficheNo,
            productName: element.product!.code,
            productBarcode: element.product!.barcode,
            warehouse: element.warehouse!.code,
            serilotType: element.product!.serilotType,
            scannedQty: element.shippedQty,
        ...
        await _dbHelper.removeOrderDetailScannedItems(response.data!.ficheNo!);
      }
      _showLoadingScreen(false, "Veriler Güncelleniyor ...");
      setState(() {
        isAssing = isAssingRemote;
        isAssingedPersonIsCurrenUser = true;
      });
...
//await Future.delayed(Duration(seconds: 3));
  ...
  OrderHeader _createOrderSummarLocalModel() {
    return OrderHeader(
        widget.item.orderId!,
        widget.item.ficheNo!,
        widget.item.ficheDate!.millisecondsSinceEpoch,
        widget.item.customer,
        widget.item.shippingMethod!,
        widget.item.warehouse!,
        widget.item.lineCount!,
        widget.item.orderStatus!,
        widget.item.totalQty!,
        widget.item.total!,
        widget.item.isAssing! == true ? 1 : 0,
        widget.item.assingmentEmail!,
        widget.item.assingCode!,
        widget.item.assingmetFullname!);
  }
}
