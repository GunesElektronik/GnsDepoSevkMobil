...
class WaybillsOrderDetail extends StatefulWidget {
  WaybillsOrderDetail(
      {super.key,
      required this.waybillsId,
  ...
  @override
  State<WaybillsOrderDetail> createState() => _WaybillsOrderDetailState();
}

class _WaybillsOrderDetailState extends State<WaybillsOrderDetail> {
  ...
  Future<void> _getOrderDetailReponse() async {
    ...
    for (int i = 0; i < result.length; i++) {
      var response =
          await apiRepository.getPurchaseOrderDetail(result[i].orderId!);
      purchaseOrderDetailResponse.add(response);
    }
  }

  Future<void> _checkForAnyChangesFromLogo() async {
    for (int i = 0; i < purchaseOrderDetailResponse.length; i++) {
      await _checkForRowItemsAddOrDeleteFromLogo(
          purchaseOrderDetailResponse[i],
          purchaseOrderDetailResponse[i].order!.orderId!,
          purchaseOrderDetailResponse[i].order!.orderItems!);
  ...
  Future<void> _getLocalData() async {
    isDataFetch = false;
    var results =
        await _dbHelper.getWaybillOrderDetailItemList(widget.waybillsId!);
    ...
    for (int i = 0; i < newList.length; i++) {
      ...
      for (int j = 0; j < waybillItemLocal!.length; j++) {
        ...
        if (localElement.qty! != remoteElement.qty!.toInt() &&
          ...
          await _dbHelper.updateWaybillDetailItemQty(
              widget.waybillsId!,
  ...
  Future<bool> _checkForRowItemsAddOrDeleteFromLogo(
      ...
      String orderId,
      List<OrderItems> newList) async {
    List<OrderItems> addedItems = findAddedItems(waybillItemLocal!, newList);
    List<WaybillItemLocalModel> deletedItems =
        findDeletedItems(waybillItemLocal!, newList);

    for (OrderItems item in addedItems) {
      await _dbHelper.addWaybillOrderDetailItem(WaybillItemLocalModel(
        waybillsId: widget.waybillsId,
        orderId: orderId,
    ...
    for (WaybillItemLocalModel item in deletedItems) {
      await _dbHelper.deleteWaybillDetailItem(widget.waybillsId!, item.orderId!,
          item.orderItemId!, item.warehouseId!);
    }
  ...
  List<WaybillItemLocalModel> findDeletedItems(
      List<WaybillItemLocalModel> oldList, List<OrderItems> newList) {
    List<WaybillItemLocalModel> deletedItems = [];
        ...
        child: Column(
          children: [
            ...
            _listTileButton(
              ...
              () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => WilbillsScanPage(
                      waybillItemLocal: waybillItemLocal,
                      productBarcodesLocalList: productBarcodesLocalList,
                      ficheNo: widget.ficheNo,
                    ),
                  ),
                ).then((value) async {
                  var results = await _dbHelper.getWaybillOrderDetailItemList(
                      widget.waybillsId.toString());
                  waybillItemLocal = results;
                  setState(() {});
                });
              },
            ...
            _listTileButton("Sevk Et", Icons.fire_truck, Colors.black,
              ...
              bool isWaybillCreated = await apiRepository
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return _selectingAreaRowItem(warehouse[index].code!,
                            ...
                            for (int i = 0; i < waybillItemLocal!.length; i++) {
                              await _dbHelper.updateWaybillDetailItemWarehouse(
                                  widget.waybillsId!,
                            ...
                            waybillItemLocal =
                                await _dbHelper.getWaybillOrderDetailItemList(
                                    widget.waybillsId!);
                            setState(() {
                              isLoading = false;
                            });
                            ...
                            await _dbHelper
                                .updateMultiPurchaseOrderOrderWarehouse(
                                    widget.waybillsId!,
                                    workplaceId,
                                    departmentId,
                                    warehouseId,
...
//burası
  ...
  Future<List<WaybillItemsNew>> _createWaybillItemListNew() async {
    ...
    // Geleneksel for döngüsü kullanarak async işlemleri yönetiyoruz
    for (int i = 0; i < waybillItemLocal!.length; i++) {
      ...
      if (element.scannedQty! > 0) {
        ...
        if (isProductLocation) {
          scannedList = await _dbHelper.getWaybillOrderScannedItem(
              element.recid!, widget.waybillsId!, element.productId!);
        }
  ...
  List<WaybillItemDetails> _createWaybillItemDetails(
    ...
    purchaseOrderDetailResponse.forEach((salesResponse) {
