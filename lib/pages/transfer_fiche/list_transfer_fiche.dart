...
import 'package:gns_warehouse/pages/transfer_fiche/detail_transfer_fiche.dart';

import 'package:gns_warehouse/repositories/apirepository.dart';

class FicheTransferList extends StatefulWidget {
  const FicheTransferList({super.key});

  @override
  State<FicheTransferList> createState() => _FicheTransferListState();
}

class _FicheTransferListState extends State<FicheTransferList> {
  late ApiRepository apiRepository;
  late TextEditingController _searchController;
  late TransferFicheListResponse? transferFicheListResponse;
  ...
  _createApiRepository() async {
    ...
    transferFicheListResponse = await apiRepository.getTransferFicheList(1, "");
    transferFicheItems = transferFicheListResponse?.transferFiches?.items ?? [];
    setState(() {
      isFetched = true;
    });
  }
  ...
  void _filterOrderUsingService(String newQuery) async {
    ...
    var newResponse =
        await apiRepository.getTransferFicheList(transferFichePage, newQuery);
    transferFicheItems =
        transferFicheItems + newResponse!.transferFiches!.items!;
                    ...
                    child: SingleChildScrollView(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return _row(transferFicheItems[index]);
                        },
                      ),
                    ),
                  ),
              ...
              onTap: () {
                Navigator.push(
                  ...
                  transferFicheListResponse =
                      await apiRepository.getTransferFicheList(1, "");
                  transferFicheItems =
                      transferFicheListResponse?.transferFiches?.items ?? [];
  ...
  Padding _row(TransferFicheItems item) {
    return Padding(
        ...
        child: ListTile(
          ...
          onTap: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => TransferFicheDetail(
                          itemId: item.transferFicheId.toString(),
                        ))).then((value) async {});
          },
          onLongPress: () {},
          dense: true,
