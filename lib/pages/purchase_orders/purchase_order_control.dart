...
class PurchaseOrderControl extends StatefulWidget {
  const PurchaseOrderControl({super.key});

  @override
  State<PurchaseOrderControl> createState() => _PurchaseOrderControlState();
}

class _PurchaseOrderControlState extends State<PurchaseOrderControl> {
  ...
  List<IsSelectedclassPurchaseOrderSummaryItem> selectedPurchaseOrders = [];
  List<IsSelectedclassPurchaseOrderSummaryItem> selectedOrders = [];
  ...
  List<IsSelectedclassPurchaseOrderSummaryItem> denemeselected = [];
  ...
  Future<void> createApiRepository() async {
    ...
    if (userDefaultWarehouseIn.isNotEmpty) {
      await _reverseWarehouse(userDefaultWarehouseIn.toLowerCase());
    }
  ...
  Future<void> _getUserSpecialWarehouseSettings() async {
    ...
    if (sharedDefaultWarehouse.isNotEmpty) {
      userDefaultWarehouseIn = jsonDecode(sharedDefaultWarehouse);
    }
  ...
  void _getOrderBasedOnCustomer() async {
    ...
    purchaseResponse =
        await apiRepository.getPurchaseOrderSummaryListBasedOnCustomer(
            customerName, searchBasedOnFicheNo);
    selectedPurchaseOrders = [];
    purchaseResponse!.forEach(
      (element) {
        selectedPurchaseOrders
            .add(IsSelectedclassPurchaseOrderSummaryItem(element, false));
      },
    );
                                          ...
                                          children: [
                                            ListView.builder(
                                              ...
                                              itemBuilder: (context, index) {
                                                ...
                                                if (isSelected) {
                                                  selectedPurchaseOrders[index]
                                                      .isSelected = true;
                                                }
                                                return OrderControlCard(
                                                  item: selectedPurchaseOrders[
                                                      index],
