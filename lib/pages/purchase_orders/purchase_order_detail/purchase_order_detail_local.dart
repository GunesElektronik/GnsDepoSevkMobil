...
class PurchaseOrderDetailLocal extends StatefulWidget {
  const PurchaseOrderDetailLocal({super.key, required this.item});

  final PurchaseOrderSummaryItem item;
  @override
  State<PurchaseOrderDetailLocal> createState() =>
      _PurchaseOrderDetailLocalState();
}

class _PurchaseOrderDetailLocalState extends State<PurchaseOrderDetailLocal> {
  ...
  void _getLocalOrderDetailItemList() async {
    var results =
        await _dbHelper.getPurchaseOrderDetailItemList(widget.item.orderId!);
                            ...
                            MaterialPageRoute(
                                builder: (context) => PurchaseOrderScanPage(
                                      purchaseOrderDetailItemList:
                                          orderDetailItemList,
                                      purchaseOrderId: widget.item.orderId!,
                                    ))).then((value) async {
                          var results =
            ...
            _listTileButton(
              ...
              Navigator.push(
                ...
                var results = await _dbHelper
                    .getPurchaseOrderDetailItemList(widget.item.orderId!);
                orderDetailItemList = results;
                setState(() {});
              });
            }),
