...
import 'package:gns_warehouse/pages/purchase_orders/components/purchase_order_item_row.dart';
...
class PurchaseOrderScanPage extends StatefulWidget {
  PurchaseOrderScanPage({
    super.key,
    required this.purchaseOrderDetailItemList,
    required this.purchaseOrderId,
  });
  final List<PurchaseOrderDetailItemDB>? purchaseOrderDetailItemList;
  String purchaseOrderId;

  @override
  State<PurchaseOrderScanPage> createState() => _PurchaseOrderScanPageState();
}

class _PurchaseOrderScanPageState extends State<PurchaseOrderScanPage> {
  ...
  bool isFindAnyExtraBarcode = false;
  @override
  void initState() {
  ...
  _createApiRepository() async {
    ...
    productBarcodesLocalList = await _dbHelper.getProductBarcodes(widget.purchaseOrderId);
  ...
  bool _isMatchWithEnteredBarcode(String barcode) {
    if (barcode.isNotEmpty) {
      for (int i = 0; i < widget.purchaseOrderDetailItemList!.length; i++) {
        if (widget.purchaseOrderDetailItemList![i].productBarcode == barcode) {
          return true;
        }
      }
      return false;
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                //         primary: false,
                //         shrinkWrap: true,
                //         itemCount: widget.purchaseOrderDetailItemList!.length,
                //         itemBuilder: (context, index) {
                //           return PurchaseOrderProductItemRow(
                //             item: widget.purchaseOrderDetailItemList![index],
                //             index: index,
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      children: widget.purchaseOrderDetailItemList!.map((item) {
                        int index = widget.purchaseOrderDetailItemList!.indexOf(item);
                        return PurchaseOrderProductItemRow(
                          item: item,
                          index: index,
                          scannedBarcode: _scannedBarcode,
  ...
  bool _anyBarcodeNoMatch() {
    for (int i = 0; i < widget.purchaseOrderDetailItemList!.length; i++) {
      if (widget.purchaseOrderDetailItemList![i].productBarcode == _scannedBarcode) {
        return true;
      }
    }
  ...
  Padding _fisnoText() {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(
            "${widget.purchaseOrderDetailItemList![0].ficheNo}",
  ...
  AppBar _appBar(BuildContext context) {
    return AppBar(
      ...
      leading: IconButton(
        onPressed: () async {
          print("orderId: ${widget.purchaseOrderId}");
          scanQRCode();
          /*
          late ApiRepository apiRepository;
