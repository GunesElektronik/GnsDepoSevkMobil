...
import 'package:gns_warehouse/pages/multi_sales_order/components/multi_sales_order_list_item.dart';
...
class CheckListMultiSalesOrder extends StatefulWidget {
  ...
  List<IsSelectedclassSalesOrderSummaryItem> selectedItems;
  ...
  @override
  State<CheckListMultiSalesOrder> createState() =>
      _CheckListMultiSalesOrderState();
}

class _CheckListMultiSalesOrderState extends State<CheckListMultiSalesOrder> {
                        ...
                        children: [
                          ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return MultiSalesOrderListItem(
                                item: widget.selectedItems[index],
                                departments: widget.departments,
                                response: (value) {
                                  salesOrderDetailResponse.add(value);
                                },
                                changedItem: (value) {
                                  for (int i = 0;
                                      i < salesOrderDetailResponse.length;
                                      i++) {
                                    if (salesOrderDetailResponse[i]
                                            .order!
                                            .ficheNo ==
                                        value.order!.ficheNo) {
                                      salesOrderDetailResponse[i] = value;
                                    }
                                  }
                                },
                              );
  ...
  WayybillsRequestBody _createWaybillBody() {
    return WayybillsRequestBody(
      ...
      salesmanId: salesOrderDetailResponse[0].order?.salesman?.salesmanId ?? "",
      waybillStatusId: 1,
      erpId: "",
      erpCode: "",
  ...
  List<WaybillItems> _createWaybillItemList() {
    ...
    salesOrderDetailResponse.forEach((orderElement) {
  ...
  WaybillLocalModel _createWaybillLocal() {
    return WaybillLocalModel(
      ...
      widget.customerId,
      widget.customerName,
      salesOrderDetailResponse[0].order?.salesman?.salesmanId,
      widget.ficheDate,
      widget.shipDate,
      widget.workplaceId,
      widget.departmentId,
      ...
      1,
      salesOrderDetailResponse[0].order?.currencyId,
      _sumOfTotalDiscounted(),
      _sumOfTotalVat(),
      _sumOfTotalGrossTotal(),
      salesOrderDetailResponse[0].order?.shippingAccount?.customerId,
      salesOrderDetailResponse[0].order?.shippingAddress?.shippingAddressId,
      salesOrderDetailResponse[0].order?.description,
    );
  }

  double _sumOfTotalDiscounted() {
    double total = 0;
    salesOrderDetailResponse.forEach((element) {
  ...
  Future<void> _setAssingStatus() async {
    for (int i = 0; i < widget.selectedItems.length; i++) {
      await apiRepository.setOrderAssingStatus(true, apiRepository.employeeUid,
          widget.selectedItems[i].item.orderId!);
    }
    // await apiRepository.getPurchaseOrderSummaryList();
  }

  Future<void> _localSave() async {
    await _dbHelper.addMultiSalesOrder(_createWaybillLocal());
  ...
  _getProductBarcodes(String salesOrderId, String productId) async {
